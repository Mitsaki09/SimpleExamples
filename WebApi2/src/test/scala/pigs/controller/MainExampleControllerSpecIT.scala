...
class MainExampleControllerSpecIT extends AsyncFlatSpec with Suite with Matchers with BeforeAndAfterAll with ScalaFutures with Eventually {
  ...
  "/example/jsonbody/car" should "должен вощврощать нормальный результат" in {
    implicit val system = ActorSystem()
    ...
    val resultFuture = Http()
      .singleRequest(
        HttpRequest(
          method = HttpMethods.GET,
          uri = "http://localhost:8080/api/v1/example/jsonbody/car"
        )
      )
    ...
    resultFuture.map { result =>
      ...
      Await.result(Unmarshal(result).to[Car], Duration.Inf) shouldBe Car(Some(15), 1500.25, "BMV")
    }
  }
  ...
  override def beforeAll(): Unit = {
    val main = new PigsProject // Создаем класс, который запускает приложение
